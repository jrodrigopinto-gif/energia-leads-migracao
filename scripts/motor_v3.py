...
def carregar_bdgd(arquivo1, arquivo2=""):
    ...
    mapeamento = {
        "cnpj":        ["cnpj", "num_cpf_cnpj", "cpf_cnpj", "cpf_cnpj_titular", "cnpj_cpf",
                        "nr_cpf_cnpj", "nr_cnpj", "doc_cnpj", "cnpj_cpf_titular"],
...
def cruzar_bdgd_cnpj(df_bdgd, df_cnpj):
    ...
    if not tem_cnpj_bdgd:
        log("Tentativa 0: pulada — BDGD não tem coluna CNPJ (verifique mapeamento)", "AVISO")
    ...
    resultado = pd.concat([df for df in [t1, t2] if len(df) > 0], ignore_index=True)
